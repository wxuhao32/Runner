    ...
    <>
      <div className={containerClass}>
        ...
        <div className="absolute top-5 left-1/2 transform -translate-x-1/2 text-sm md:text-lg text-purple-300 font-bold tracking-wider font-mono bg-black/50 px-3 py-1 rounded-full border border-purple-500/30 backdrop-blur-sm z-50">
          {mode === 'ENDLESS' ? (
            <>
              {t(lang, 'ui.hud.endless')} <span className="text-white">∞</span>
            </>
          ) : (
            <>
        ...
        {/* Buff Bar */}
        {(shieldCharges > 0 || isMagnetActive || isControlsInverted || scoreMultiplier > 1 || speedMultiplier !== 1) && (
          <div className="absolute top-16 right-4 md:right-8 flex flex-col gap-2 pointer-events-none">
            {shieldCharges > 0 && (
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/55 border border-cyan-400/30 backdrop-blur-sm text-cyan-200 font-mono text-xs md:text-sm">
                <Shield className="w-4 h-4" /> x{shieldCharges}
              </div>
            )}
            {isMagnetActive && (
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/55 border border-purple-400/30 backdrop-blur-sm text-purple-200 font-mono text-xs md:text-sm">
                <Magnet className="w-4 h-4" /> MAGNET
              </div>
            )}
            {scoreMultiplier > 1 && (
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/55 border border-yellow-400/30 backdrop-blur-sm text-yellow-200 font-mono text-xs md:text-sm">
                <Sparkles className="w-4 h-4" /> x{scoreMultiplier}
              </div>
            )}
            {speedMultiplier !== 1 && (
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/55 border border-blue-400/30 backdrop-blur-sm text-blue-200 font-mono text-xs md:text-sm">
                {speedMultiplier > 1 ? <Rocket className="w-4 h-4" /> : <Clock className="w-4 h-4" />}
                x{speedMultiplier.toFixed(2)}
              </div>
            )}
            {isControlsInverted && (
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/55 border border-pink-400/30 backdrop-blur-sm text-pink-200 font-mono text-xs md:text-sm">
                <Shuffle className="w-4 h-4" /> INVERT
              </div>
            )}
          </div>
        )}
